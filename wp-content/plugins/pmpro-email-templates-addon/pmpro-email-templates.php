	...
	*/
	...
	function pmproet_email_data($data, $email) {
	    ...
	    if(!empty($user->membership_level) && !empty($user->membership_level->enddate))
	        $new_data["membership_change"] .= ". " . sprintf(__("This membership will expire on %s.", "pmproet"), date('Y-m-d', strtotime(get_option('date_format'), $user->membership_level->enddate)));
