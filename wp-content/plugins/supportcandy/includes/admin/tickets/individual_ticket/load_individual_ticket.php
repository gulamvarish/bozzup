...
}else if($_SESSION['user_type']=='subscriber'){
	$approval = 'Waiting for your approval';
}

$arrayTicketstatus = array('3' =>'OPEN (TO SEND)', '4'=>$approval, '6'=>'Approved', '74'=>'Rejected', '75'=>'Modification Requested' );

$arrayTicketstatuscolor = array('3' =>'garey', '4'=>'yellow', '6'=>'green', '74'=>'red', '75'=>'blue' );
...
$company_name			   = stripcslashes($ticket['customer_name']);
...
<div class="row wpsc_tl_action_bar" >
	...
	<div class="col-sm-12">
		...
		<?php if ($current_user->ID):?>
			<button type="button" id="wpsc_individual_ticket_list_btn" onclick="wpsc_get_ticket_list();" class="btn btn-sm wpsc_action_btn" style="<?php //echo $action_default_btn_css?>"><i class="fa fa-list-ul"></i> <?php _e('Order List','supportcandy')?></button>
		<?php endif;?>
		...
		<?php
		/* HIDE CLONE
		if ($wpscfunction->has_permission('add_note',$ticket_id) && $ticket_status):?>
...
<div class="row" style="padding-top: 10px;color:<?php echo $general_appearance['wpsc_text_color']?> !important;">
  ...
  <div class="col-sm-8 col-md-8 wpsc_it_body">
      ...
      <h4>
		...
		/*	,
			...
			foreach ($threads as $thread):
				...
				$order_status   = get_post_meta( $thread->ID, 'order_status', true);
				...
				if ($thread_type == 'report'):
					...
					<div class="wpsc_thread reported" style="background-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_report_thread_bg_color']?> !important;color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_report_thread_text_color']?> !important;border-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_report_thread_border_color']?> !important;">
						...
						<div class="thread_body">
							<div class="thread_user_name reported">
								...
								<?php }
									...
									/*Added by Gulam*/
									  ...
									  $userid = get_user_by( 'email', $customer_email);

									  
                    $userId = $userid->ID;
                  
							?><strong><?php echo get_user_meta($userId, 'company', true); ?> - </strong><small><?php echo $customer_name; ?></small>
							...
							<small><i><?php echo $customer_email?> | <?php echo $date?></i></small><br>
							...
							<?php 
								...
								?>
								    ...
								    if($order_status){
										...
										<a href="javascript:void(0)" class="btn ticketstatuscolor <?php echo $arrayTicketstatuscolor[$order_status]; ?>"><?php echo $arrayTicketstatus[$order_status]; ?></a>
										<?php 
										   } //	/*Added By gulam */
										  ?>
									</div>
				...
				if ($thread_type == 'reply'):
					...
					if($wpsc_thread_date_format == 'timestamp'){
						$date = sprintf( __('replied %1$s','supportcandy'), date('Y-m-d', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
						$roles = "replied";
					}else{
						$date = sprintf( __('replied %1$s','supportcandy'), date('Y-m-d', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
						$roles = "replied";
					...
					<?php /* HIDE GRAVATOR IMAGE echo get_avatar( $customer_email, 40 ) */
								    ...
								    /* if (in_array("supplier", $user_info->roles))
										  {
										   $roles = "reported";
										  }else{
										  	$roles = "replied";
							...
							?>
					<div class="wpsc_thread <?php echo  $roles; ?>" style="<?php echo $style;?>">
						<!-- <div class="thread_avatar"> -->
							
						<!-- </div> -->
						<div class="thread_body">
							<div class="thread_user_name  <?php echo  $roles; ?>">
								<?php if ( apply_filters('wpsc_thread_email_visibility',$current_user->has_cap('wpsc_agent')) ) {?>
									
								<?php }?><strong><?php echo get_user_meta($user_info->ID, 'company', true); ?><?php //echo $customer_email?> - </strong><small><?php echo $customer_name; ?></small>
                     ...
                     <?php 
										...
										if( $seen && $seen != 'null' ){ ?>
											...
											$orderstatus1      = get_post_meta( $thread->ID, 'order_status',true);
				...
				if ( $thread_type == 'note' && apply_filters('wpsc_private_note_visibility',$current_user->has_cap('wpsc_agent'), $thread) &&  $wpscfunction->has_permission('view_note',$ticket_id) ):
					...
					<div class="wpsc_thread note" style="background-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_private_note_bg_color']?> !important;color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_private_note_text_color']?> !important;border-color:<?php echo $wpsc_appearance_individual_ticket_page['wpsc_private_note_border_color']?> !important;">
						...
						<?php 
						if($wpsc_thread_date_format == 'timestamp'){
							$date = sprintf( __('added note %1$s','supportcandy'),date('Y-d-m', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
						}else{
							$date = sprintf( __('added note %1$s','supportcandy'), date('Y-d-m', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
						}
						?>
				...
				if ( $thread_type == 'log' && apply_filters('wpsc_thread_log_visibility',$current_user->has_cap('wpsc_agent')) && $wpscfunction->has_permission('view_log',$ticket_id)):
		          ...
		          <?php 
							if($wpsc_thread_date_format == 'timestamp'){
								$date = sprintf( __('reported %1$s','supportcandy'),date('Y-d-m', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
							}else{
								$date = sprintf( __('reported %1$s','supportcandy'), date('Y-d-m', strtotime($wpscfunction->time_elapsed_timestamp($thread->post_date_gmt))) );
							}
			...
			<?php
			$ticketstatus = array('6', '74' );

		if($reply_form_position && $ticket_status && !in_array($status_id, $ticketstatus)){
	...
	<div class="col-sm-4 col-md-3 wpsc_sidebar individual_ticket_widget">
		...
		foreach ($ticket_widgets as $key => $ticket_widget): 
			...
			?>
				...
				<?php 
					if ( $ticket_widget->slug == "status_HIDE_DEFAULT" && $flag ):
							...
							<div class="row" id="wpsc_ticket_fields_widget" >
								...
								<div id="wpsc_ticket_fields">
									...
									<div class="wpsp_sidebar_labels">
										<strong>Company Name:</strong> <?php echo $company_name; ?>
									</div>
					    	</div>
...
<script>
	...
	// This function waits for status change and then trigger form submit function.
	function wpsc_before_submit_reply(status_code) {



		var dataform = new FormData(jQuery('#wpsc_frm_tkt_reply')[0]);

      	// status is save and send later
      	if(status_code == 3) {
      		if(!confirm('Save and send later. Are you sure?')) return false;
      	}else if(status_code == 4) {
      		if(!confirm('Send for approval. Are you sure?')) return false;
      		...
      		var desc_content = tinyMCE.activeEditor.getContent().trim();
		  ...
		  if( (status_code==6 || status_code==74) ){ 
			if(dataform.has('desc_attachment[]')) {
				alert('Attachment can not be added');
				return false;
			}

			 var desc_content = tinyMCE.activeEditor.getContent().trim();

				if(desc_content !="" || desc_content.length > 0) {
					alert('Description can not be added');
					return false;
				}

      		jQuery('#mceu_11, .attachment_link').hide();
      	}else{
      		jQuery('#mceu_11').show();
      	}

		jQuery("#cstm_statusvalue").val(status_code);

		setTimeout(() => {
			wpsc_submit_reply( 'reply' );       		
     	}, 1000);
	...
	function wpsc_submit_reply( save_type ){
		 ...
		 /*Added By Gulam */
      var status_code = jQuery(".cstm_statusvalue").val();
     
      	if(status_code ==0){
      	    alert('<?php _e('Select Order Status!','supportcandy')?>');
			return;
	 	}
		 /*Added By Gulam */
		...
		if((in_array('register_user',$wpsc_allow_rich_text_editor) && !$current_user->has_cap('wpsc_agent')) && $rich_editing){
			...
			var myArray = {75: 'Request modification. Are you sure?', 74: 'Reject. Are you sure?', 6: 'Approve. Are you sure?', 4: 'are you sure? Sent for Approval Description required, mandatory', 3: 'are you sure? Open(To send) Description required, mandatory'};
			<?php 
			// permission updated by rkumar
				if($current_user->has_cap('wpsc_agent')) { ?>
					//console.log('has permission');
					
					// save and send later and send for approval.
				...
				<?php } else { ?>
					...
					var dataform = new FormData(jQuery('#wpsc_frm_tkt_reply')[0]);
			             ...
			             // console.log("status code", status_code);
			            	...
			            	var regex = /(<([^>]+)>)/ig
							var des_length = !!description.replace(regex, "").length;
							//console.log("length", des_length, "description val ", description);

							//75 for modification request
							if(status_code == 75 ){
								if(!confirm(myArray[status_code])) return;
								...
								if( (status_code == 74 || status_code == 6) && des_length) {
									alert('Description can not be added');
									return false;
								}
								// 74 for reject, 6 for approve, .
								// console.log("length", des_length, "description val ", description);
	...
	function wpsc_post_reply(description){
		<?php if($wpsc_allow_reply_confirmation){
			...
			//var myArray = {75: 'Request modification, are you sure? Description required, mandatory', 74: 'Reject, are you sure? It must be impossible to write a Description', 6: 'Approve, are you sure?  It must be impossible to write a Description', 4: 'are you sure? Sent for Approval Description required, mandatory', 3: 'are you sure? Open(To send) Description required, mandatory'};
//alert(myArray[status_code]);
