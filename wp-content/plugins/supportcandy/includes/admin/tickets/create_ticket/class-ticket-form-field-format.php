...
if ( ! class_exists( 'WPSC_Ticket_Form_Field' ) ) :
    ...
    class WPSC_Ticket_Form_Field {
		...
		function print_drop_down($field){
          ...
          <div  data-fieldtype="text" data-visibility="<?php echo $this->visibility_conditions?>" class="<?php echo $this->col_class?> <?php echo $this->visibility? 'hidden':'visible'?> <?php echo $this->required? 'wpsc_required':''?> form-group wpsc_form_field <?php echo 'field_'.$field->term_id?>">
            <label class="wpsc_ct_field_label" for="<?php echo $this->slug;?>">
              <?php //echo $this->label['custom_fields_'.$field->term_id];?> 

              <?php if($this->slug == 'customer_name'): ?>
              			<?php echo 'Company Name'; ?>
              		<?php else : ?>
              			<?php echo $this->label['custom_fields_'.$field->term_id]; ?>
              	<?php endif; ?>

              <?php echo $this->required? '<span style="color:red;">*</span>':'';?>
            </label>
            <?php if($this->extra_info['custom_fields_extra_info_'.$field->term_id]){?><p class="help-block" style="<?php echo $extra_info_css?>"><?php echo $this->extra_info['custom_fields_extra_info_'.$field->term_id];?></p><?php }?>
						<input type="text" id="<?php echo $this->slug;?>" class="form-control wpsc_customer_name" placeholder="<?php echo $this->placeholder; ?>" name="<?php echo $this->slug;?>" autocomplete="off" 
						value="" <?php echo $readonly ?>  onkeypress="wpsc_text_limit(event,this,<?php echo $this->limit ?>);">
					</div>	
					<?php
				}
				...
				function print_customer_email($field){
					...
					// Can't change customer email address
					$readonly = 'readonly';
					if(!$current_user->has_cap('wpsc_agent') && is_user_logged_in()){
						$readonly = 'readonly';
					}
          ...
          <div  data-fieldtype="email" data-visibility="<?php echo $this->visibility_conditions?>" class="<?php echo $this->col_class?> <?php echo $this->visibility? 'hidden':'visible'?> <?php echo $this->required? 'wpsc_required':''?> form-group wpsc_form_field <?php echo 'field_'.$field->term_id?>">
            ...
            <?php if($this->extra_info['custom_fields_extra_info_'.$field->term_id]){?><p class="help-block" style="<?php echo $extra_info_css?>" ><?php echo $this->extra_info['custom_fields_extra_info_'.$field->term_id];?></p><?php }?>
						<input type="text" id="<?php echo $this->slug;?>" class="form-control wpsc_email" placeholder= "<?php echo $this->placeholder;?>" name="<?php echo $this->slug;?>" autocomplete="off" value="" 
            ...
            <?php if($this->extra_info['custom_fields_extra_info_'.$field->term_id]){?><p class="help-block" style="<?php echo $extra_info_css?>" ><?php echo $this->extra_info['custom_fields_extra_info_'.$field->term_id];?></p><?php }?>
						<div class="row attachment" style="margin-bottom:20px;">
	  					...
	  					<div class="row attachment_link">
	  						<span onclick="wpsc_attachment_upload('<?php echo 'attach_'.$field->term_id?>','<?php echo $this->slug?>');"><i><img src="<?php echo plugin_dir_url('/').'supportcandy/asset/images/icon/attachment.png'; ?>"></i><?php _e('Attach file','supportcandy')?></span>
	  					</div>
	  				</div>
          </div>
          <?php
          ...
          <?php
            ...
            <label class="wpsc_ct_field_label">
              <?php echo $this->label['custom_fields_'.$field->term_id];?> <?php echo $this->required = false? '<span style="color:red;">*</span>':'';?>
            </label>
            <?php if($this->extra_info['custom_fields_extra_info_'.$field->term_id]){?><p class="help-block" style="<?php echo $extra_info_css?>"><?php echo $this->extra_info['custom_fields_extra_info_'.$field->term_id];?></p><?php }?>
            ...
            <textarea id="<?php echo $this->slug;?>" class="wpsc_textarea" name="<?php echo $this->slug;?>" placeholder ="<?php echo $this->placeholder; ?>" ></textarea>
						    ...
						    <div class="row attachment" style="margin-bottom:20px;margin-top:20px;">
		  					<div class="row attachment_link">
								<?php 
								$notice_flag = false;
								if( (in_array('customers',$wpsc_allow_attach_create_ticket) && is_user_logged_in() && !$current_user->has_cap('wpsc_agent')) ||
									...
									<div style="margin-bottom: 15px;">
										<label class="wpsc_ct_field_label" > Upload artwork to approve</label>
									</div>
									<span onclick="wpsc_attachment_upload('<?php echo 'attach_'.$field->term_id?>','desc_attachment');"><i><img src="<?php echo plugin_dir_url('/').'supportcandy/asset/images/icon/attachment.png'; ?>"></i><?php _e('Attach file','supportcandy')?></span>
									<?php
								} 
								?>
									<?php
