...
include WPSC_ABSPATH . 'includes/admin/ticket_list/set_leftsidebar.php';
?>
...
<div id="create_ticket_body" class="row" >
	<div class="row">
	<div class="col-sm-6 headingcol"><h4 class="headingcreateticket">New Order</h4></div>
  <div class="col-sm-6 text-right btncol">
    <!-- <button type="button" id="wpsc_load_new_create_ticket_btn" onclick="wpsc_get_create_ticket();" class="btn btn-sm wpsc_create_ticket_btn" style="<?php echo $create_ticket_btn_css?>"><i class="fa fa-plus"></i> <?php _e('New Order','supportcandy')?></button> -->
    	<button type="button" id="wpsc_create_ticket_reset" onclick="wpsc_get_create_ticket();" class="btn" style="background-color:<?php echo $wpsc_appearance_create_ticket['wpsc_reset_button_bg_color']?> !important;"><?php _e('Reset Form','supportcandy')?></button>
    <?php if($current_user->ID):?>
			<button type="button" id="wpsc_load_ticket_list_btn" onclick="wpsc_get_ticket_list();" class="btn btn-sm wpsc_action_btn" style="<?php echo $action_default_btn_css?>"><i class="fa fa-list-ul"></i> <?php _e('Order List','supportcandy')?></button>
		<?php endif;?>
  </div>
</div>
	<form id="wpsc_frm_create_ticket" onsubmit="return wpsc_submit_ticket();" method="post">
		<div class="row create_ticket_fields_container">
			...
			<!-- <div class="col-lg-6">
				<labe l class="wpsc_ct_field_label">Client</label>-->
				<?php
					/*
					$cstm_args = array('role' => 'subscriber','orderby' => 'user_nicename', 'order' => 'ASC');
					$subscribers = get_users($cstm_args);

					// echo "<pre>"; print_r($subscribers);
					$cstm_subscriber_list = [];
					foreach ($subscribers as $user) {
						$cstm_subscriber_list[$user->user_login]['email'] = $user->user_email;
					}
					*/
				?>


				<!-- <select class="form-control" name="" id="customer_name">
					<option value=""></option>
				 </select> -->
				<?php
				/*User dropdown*/
				//	wp_dropdown_users( array( 'role' => 'Subscriber' ) );
				?>
			<!-- </div> -->

			<!-- <div class="col-lg-6">
				<label class="wpsc_ct_field_label">Email</label>
				<input type="text" name="wpsc_email" id="wpsc_email" value="test@gmail.com" />
			</div> -->
			...
			foreach ($fields as $field) {
				...
				// 15 for description
				if($field->term_id == '14' || $field->term_id == '16')
					continue;
		...
		<div class="row create_ticket_frm_submit">
			<span type="button" id="wpsc_create_ticket_submit_later" name="send_later" class="btn" value="3"><?php _e('Save And Send Later','supportcandy')?></span>

			<span type="submit" id="wpsc_create_ticket_submit" class="btn" style="background: #478E00;"><?php _e('Send for Approval','supportcandy')?></span>
			<!-- <span class="cancelorder">Cancel</span> -->
		
		  <?php do_action('wpsc_after_create_ticket_frm_btn');?>
		</div>
		
		<input type="file" id="attachment_upload" class="hidden" onchange="" role="<?php echo $_SESSION['user_type']; ?>">

		<input type="hidden" id="ticketstatusval" name="ticket_status" value="<?php echo get_option('wpsc_default_ticket_status'); ?>">
...
/*Added By Gulam*/
  ...
  jQuery("#wpsc_create_ticket_submit_later").click(function(){
  		//confirm added by rkumar
  		if(confirm('Save and send later. Are you sure?')) {
  			jQuery('#ticketstatusval').val(3);
  			wpsc_submit_ticket();          	
  		} else {
  			return false;
  		}       
  ...
  jQuery("#wpsc_create_ticket_submit").click(function(){
  		//confirm added by rkumar
  		if(confirm('Send for approval. Are you sure?')) {
    ...
    jQuery('#attachment_upload').on('change', function() {
		...
		if (!flag){
			...
			var html_str = '<div class="row wpsp_attachment">'+
				'<div class="progress attachmentprogress">'+
            ...
            data.append('nonce', jQuery('#wpsc_nonce').val().trim());
              ...
              success: function(response) {
						    ...
						    jQuery(attachment).find('.attachment_cancel').show();
								...
								/*if(jQuery('#attachment_upload').attr('role') == 'supplier'){
