	...
	if($tf_type > '0') {
		...
		switch($match){
			...
			case '{ticket_url}':
			  ...
			  // admin Company name
				case '{admin_company_name}':				
					$admincompanyname = get_user_meta($current_user->ID, 'company', true ); 
					$str = preg_replace('/{admin_company_name}/', $admincompanyname, $str);
				  break;

			  case '{btn_approved}':				
				$login_url = home_url();
				$approved_url = '<a href="'.$login_url.'" target="_blank" style="background: green; color: #fff; padding: 7px; border-radius: 4px; border: 0px solid; margin-right: 5px; text-decoration:none;">Approve</a>';
				$str = preg_replace('/{btn_approved}/', $approved_url, $str);
			  ...
			  case '{btn_modification}':				
				$login_url = home_url();
				$approved_url = '<a href="'.$login_url.'" target="_blank" style="background: blue; color: #fff; padding: 7px; border-radius: 4px; border: 0px solid; margin-right: 5px; text-decoration:none;">Request Modification</a>';
				$str = preg_replace('/{btn_modification}/', $approved_url, $str);

			  break;

			   case '{btn_reject}':				
				$login_url = home_url();
				$approved_url = '<a href="'.$login_url.'" target="_blank" style="background: red; color: #fff; padding: 7px; border-radius: 4px; border: 0px solid; margin-right: 5px; text-decoration:none;">Reject</a>';
				$str = preg_replace('/{btn_reject}/', $approved_url, $str);
