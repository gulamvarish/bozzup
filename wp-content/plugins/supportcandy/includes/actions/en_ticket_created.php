...
foreach ($email_templates as $email) :
  ...
  if( $wpscfunction->check_ticket_conditions($conditions,$ticket_id) ) :
   ...
   /* Aded By Gulam */
    //$subject          = 'Order '.$ticket_id.' '.stripslashes($subject);
    /* Aded By Gulam */
    ...
    if($wpsc_email_sending_method){
      ...
      $tablename = $wpdb->prefix . 'wpsc_ticket';

      $ticketstatus = $wpdb->get_results("SELECT `ticket_status` FROM $tablename WHERE id = '".$ticket_id."'");

      /*Added By Gulam*/
      if($ticketstatus[0]->ticket_status !=3){
