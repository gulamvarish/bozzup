	...
	if ( ! empty( $level ) ) {
		...
		if ( is_array( $level ) ) {
			...
			if($level['enddate']=="NULL"){
	        	...
	        	$wpdb->query("UPDATE $table_name SET `status`='inactive' WHERE user_id=".$user_id);
			}
...
function pmpro_getMembershipLevelsForUser( $user_id = null, $include_inactive = false ) {
	...
	if ( $levels === false ) {
		...
		$levels = $wpdb->get_results(
			...
			GROUP BY subscription_id DESC LIMIT 1" )
		);
...
/*function assign_pmpro_level_to_role($user_id, $role, $old_roles)
{
	...
	update_user_meta($user_id, 'wp_capabilities', $value);

	if(count($old_roles)==0){

		$role1 = 'pmpro_role_'.$_GET['level'];	
		$someone->add_role( $role1 );
		$someone->add_role( 'supplier' );			 
		...
		if($user->roles[0]=='subscriber' && isset($user->roles[0])){				

			$someone->add_role( $role );
