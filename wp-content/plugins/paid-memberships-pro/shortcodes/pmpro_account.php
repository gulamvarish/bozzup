							...
							<tr>
								<td class="<?php echo pmpro_get_element_class( 'pmpro_account-membership-levelname' ); ?>">
									...
									<div class="<?php echo pmpro_get_element_class( 'pmpro_actionlinks' ); ?>">
										...
										$pmpro_member_action_links['cancel'] = sprintf( '<a id="pmpro_actionlink-cancel" href="%s">%s</a>', esc_url( add_query_arg( 'levelstocancel', $level->id. "&usersubscription=".$level->subscription_id, pmpro_url( 'cancel' ) ) ), esc_html__( 'Cancel', 'paid-memberships-pro' ) );
...
//print_r($level);
								...
								<?php
									...
									$dateexit = apply_filters( 'pmpro_account_membership_expiration_text', $expiration_text, $level );

									if($dateexit == "---"){
										echo $dateexit;
									}else{
										echo date('Y-m-d', strtotime(apply_filters( 'pmpro_account_membership_expiration_text', $expiration_text, $level )));
									}
		...
		<?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="<?php echo pmpro_get_element_class( 'pmpro_box', 'pmpro_account-profile' ); ?>" style="display: none;">
