...
if ( isset ( $_REQUEST['bcompany'] ) ) {
	$bcompany = sanitize_text_field( $_REQUEST['bcompany']  );
		...
		if ( pmpro_changeMembershipLevel( $custom_level, $user_id, 'changed' ) ) {
			...
			if($sendemails) { // Send the emails only if the flag is set to true
			   ...
			   $user_meta=get_userdata($user_id);
          	   ...
          	   if (in_array("supplier", $user_meta->roles))
				  {
				     session_start();
					    $_SESSION['user_type'] = 'supplier';
					     $someone = new WP_User( $user_id );
					    $someone->add_role( 'wpsc_agent' );
				  }
				...
				if($user_meta->roles[0] == 'subscriber'){
					session_start();
				   $someone = new WP_User( $user_id );
				   $someone->add_role( 'subscriber' );
				   $someone->add_role( 'wpsc_agent' );	
				   $_SESSION['user_type'] = 'supplier';		      				  
				}else if(isset($user_meta->roles[2]) && $user_meta->roles[2]== 'subscriber'){
					session_start();
					$someone = new WP_User( $user_id );
				    $someone->add_role( 'subscriber' );
				    $someone->add_role( 'wpsc_agent' );	
			...
			if ( ! empty( $bcompany ) && ! empty($user_id) ) {
				add_user_meta( $user_id, 'company', $bcompany);

				$table_name = $wpdb->prefix . 'terms';
				$termdata =  'agent_'.$user_id;
				$term   = $wpdb->insert($table_name, array('name' => $termdata, 'slug' => $termdata));

				$termid = $wpdb->insert_id;
				$tabletaxonomy = $wpdb->prefix . 'term_taxonomy'; 
				$taxonomy      = $wpdb->insert($tabletaxonomy, array('term_id' => $termid, 'taxonomy' => 'wpsc_agents'));

				$userdetail = new WP_User( $user_id );
				add_term_meta( $termid, "email" , $userdetail->user_email );
				add_term_meta( $termid, "nicename" , $userdetail->user_nicename );
				add_term_meta( $termid, "last_name" , $userdetail->last_name );
				add_term_meta( $termid, "first_name" , $userdetail->first_name );
				add_term_meta( $termid, "agentgroup" , '0' );
				add_term_meta( $termid, "role" , '1' );
				add_term_meta( $termid, "label" , $userdetail->user_nicename );
				add_term_meta( $termid, "user_id" , $userdetail->ID );				
				update_user_option($userdetail->ID,'wpsc_agent_role',1);

				do_action('wpsc_set_add_agent',$termid);
